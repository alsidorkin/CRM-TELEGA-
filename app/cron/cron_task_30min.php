...
try{
    ...
    // поле reminder_at соответствует текущей дате и времени с погрешностью 15 минут

    $query =" SELECT tr.*, tl.title, tl.finish_date, ut.telegram_chat_id, ut.telegram_username
    FROM todo_reminders AS tr
    INNER JOIN todo_task AS tl ON tr.task_id = tl.id
    ...
    foreach($tasks as $task){
        ...
        $taskId = $task['task_id'];
        $taskLink = 'https://greenhouse.hhos.com.ua/todo/tasks/task/' . $taskId;
